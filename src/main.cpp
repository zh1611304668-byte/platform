...
void loop() {
  ...
  if (k4IsPressed && !k4ToggledThisPress && k4PressedInScreen1 &&
      safeGetCurrentScreen() == SCREEN1 && k4PressTime > 0 &&
      (now - k4PressTime) >= trainingLongPress && (now - k4PressTime) < 60000) {

    if (!training.isActive()) {
      training.start();
      Serial.println("========================================");
      Serial.println("✓ [训练模式] 长按触发 - 开始训练");
      Serial.println("========================================");
    } else {
      training.stop();
      Serial.println("========================================");
      Serial.println("✓ [训练模式] 长按触发 - 停止训练");
      Serial.println("========================================");
    }

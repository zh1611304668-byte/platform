 ...
 * Purpose: Declares interfaces, types, and constants for the Config Manager module.
 */
...
private:
    ...
    bool make4GHttpRequestWithPort(const String& endpoint, String& response, int port);
    bool send4GATCommand(String command, String expectedResponse, unsigned long timeout = 5000);
...
public:
    ...
    bool isConfigReady() const { return deviceConfig.isValid && mqttConfig.isValid; }
    ...
    bool getDeviceInfoByAddress(const String& address, String& deviceCode, String& rowerName) const;
    ...
    void enableNITZTime();
    void setDateTime(const String& date, const String& time, const String& dateTime);
...
public:
    ...
    void safeUpdateSensorData(float newStrokeRate, float newSpeedMps, int newStrokeCount, float newTotalDistance);
    void safeGetSensorData(float& outStrokeRate, float& outSpeedMps, int& outStrokeCount, float& outTotalDistance);
